...
def PhoneStats(rdd):
	phoneNumRegex = re.compile(r'(\d{3}[-\.\s]??\d{3}[-\.\s]??\d{4}|\(\d{3}\)\s*\d{3}[-\.\s]??\d{4}|\d{3}[-\.\s]??\d{4})') # Regex needs to be changed
...
def StreetStats(rdd):
	...
	addressRegex = re.compile(r'[0-9]+[ ,]+([\w.-]+ *(st|th|rd)?)+ *(avenue|av|ave|drive|dr|road|rd|boulevard|blvd|lane|ln|street|st)\.?')
	result = rdd.map(lambda x:      True    if      (addressRegex.match(x.lower()) == None and streetRegex.match(x.lower()) != None)     else    False).filter(lambda       x:      x==True)
	total_count=rdd.count()
