...
boroughs = ['the bronx','staten island','manhattan','brooklyn','queens','bronx']
...
building_classification_df = spark.read.load("/user/nap493/Building_Classification.csv", format="csv", inferSchema="true")
...
def VehicleTypeStats(rdd):
	...
	result_count=output_df.rdd.filter(lambda x: x[2]==1).count()
	return {'semantic_type': 'Vehicle Type', 'count': result_count}
