...
def process_dataset(filename):
    ...
    count_nan_vals = input_data.select([count(when(isnan(get_col_name(column_name)), get_col_name(column_name))).alias(column_name) for column_name in input_data.columns])
    count_null_vals = input_data.select([count(when(col(get_col_name(column_name)).isNull(), get_col_name(column_name))).alias(column_name) for column_name in input_data.columns])
